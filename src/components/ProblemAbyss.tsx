...
export const ProblemAbyss: React.FC = () => {
  ...
  return (
    <section className="py-20 bg-[#0B0E14] border-t border-[#1E2638]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {items.map((item, idx) => (
            <div 
              key={idx} 
              className="p-5 rounded-2xl bg-[#0E131E] border border-[#1E2638] hover:border-slate-600 transition-all flex flex-col justify-between"
            >
              <div>
                <div className="w-10 h-10 rounded-xl bg-slate-900 border border-[#1E2638] flex items-center justify-center mb-4">
                  {item.icon}
                </div>
